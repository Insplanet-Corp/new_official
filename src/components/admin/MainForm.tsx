...
import { FilePick, Input, Radios, Row } from '@/components/admin/form';
  ...
  return (
    ...
    >
      <Row label="포트폴리오명" required>
        <Input value={v.name} onChange={(x) => set('name', x)} placeholder="메인에 출력할 포트폴리오명" />
      </Row>
      ...
      <Row label="상단 문구" required>
        <Input value={v.headline} onChange={(x) => set('headline', x)} placeholder="메인 상단 문구" />
      </Row>

      <Row label="하단 문구" required>
        <Input value={v.subline} onChange={(x) => set('subline', x)} placeholder="메인 하단 문구" />
      </Row>

      <Row label="클라이언트" required>
        <Input value={v.client} onChange={(x) => set('client', x)} placeholder="메인에 출력할 클라이언트" size="medium" />
      </Row>

      <Row label="오픈일" required hint="예: Jan,2026">
        <Input value={v.launch} onChange={(x) => set('launch', x)} placeholder="Jan,2026" size="short" />
      </Row>
      ...
      <div className={s.actions}>
        ...
        <button type="button" className={kit.btn} onClick={() => router.push('/admin/main')}>
          취소
        </button>
      </div>
    </form>
