...
/** PC FilteredInput 의 .mc-* 판. 로직은 1:1 동일 — 클래스명만 mobile-contact.html 기준으로 바뀐다. */
export default function MobileFilteredInput({
  inputRef,
  type,
  inputMode,
  placeholder,
  ...
  return (
    <div className="mc-cell" ref={cellRef}>
      <input
        ref={inputRef}
        className="mc-input"
        type={type}
      ...
      />
      <span className="mc-hint" role="alert">
        <span className="mc-hint-ico" aria-hidden="true">
          !
        </span>
        <span className="mc-hint-msg">{hint}</span>
      </span>
    </div>
  );
}
