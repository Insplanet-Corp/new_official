...
### 개인정보 보관·파기 — 방침과 구현이 어긋나 있다 (2026-09-02 확인)

⚠️ **사이트가 이미 공개적으로 약속해 놓은 보유기간이 있는데, 그것을 실행하는 코드가
하나도 없다.** `data/legal.ts` 의 개인정보처리방침 본문 그대로:

| 항목 | 방침이 약속한 보유 기간 | 실제 구현 |
| --- | --- | --- |
| 견적 문의 (`quotes` + `quote` 버킷) | **접수 후 1년** | 없음 — 영구 보관 |
| 입사 지원 (`recruits` + `recruit` 버킷) | **제출 후 3년 또는 채용 절차 종료 시** | 없음 — 영구 보관 |

방침 07 조는 "보유 기간의 경과 … 지체 없이 파기", "전자적 파일 : 재생할 수 없는 기술적
방법으로 영구 삭제" 라고 적혀 있다. 이는 곧 **개인정보 보호법 제21조**(불필요해지면
지체 없이 파기, 시행령상 정당한 사유가 없으면 5일 이내)와 **제30조**(방침에 적은 대로
처리할 의무)를 그대로 옮긴 문장이다. 지금은 두 화면 어디에도 **삭제 버튼조차 없다.**

⚠️ **Careers 입사지원 폼에는 개인정보 동의 절차가 아예 없다.** 견적문의 폼에는
`ConsentLinks` + 체크박스가 있는데(필수값 게이팅에도 포함), `RecruitModal` /
`MobileRecruitModal` 에는 그 어떤 동의 문구도 없다 — 이력서는 사진·생년월일 등이 담기는
가장 민감한 자료인데 수집 근거가 화면에 없다. 폼을 손볼 때 같이 붙일 것.

**붙일 때의 설계 메모** — 지우는 대상이 **DB 행과 Storage 파일 두 곳**이라는 점이 핵심이다.
`quotes`/`recruits` 행만 지우면 파일이 고아로 남고(버킷 용량은 그대로 찬다), 파일만
지우면 화면에 파일명이 남아 누르면 400 이 난다. **파일 먼저, 행 나중** 순서로 지워야
"행은 지웠는데 파일이 남는" 경우를 안 만든다.
- 018·023 에 어드민 `delete` 정책이 이미 있어 화면에서 바로 지울 수 있다(테이블·Storage 둘 다).
- 자동 파기는 Vercel cron(`/api/keepalive` 옆)에 하루 한 번 도는 라우트를 두는 것이
  가장 간단하다. ⚠️ 그 라우트는 **service_role** 로 돌아야 한다(anon 은 delete 권한이 없다).
- 파기도 기록을 남기는 것이 좋다 — `quote_access_logs.action` 에 `'purge'` 를 더하면
  같은 표에서 "누가 언제 무엇을 지웠나" 까지 되짚힌다(023 이 제약을 정의로 찾아 바꾸므로
  같은 방식으로 늘리면 된다).
- ⚠️ **`scripts/storage.mjs backup` 산출물은 파기 대상에서 빠진다.** 로컬에 받아 둔 사본은
  법적으로도 같은 개인정보다 — 백업을 뜬 뒤에는 보관 위치와 파기 시점을 사람이 관리해야 한다.
...
20. **⚠️ `object-position: left` 은 여백이 이미지 **안에** 박혀 있으면 아무 일도 하지 않는다**
    ...
    ⚠️ **파일에서 여백을 빼는 방식으로 고치지 말 것** — 같은 CI 를 PC 표(`.pj-logo`)도 쓰는데
    그쪽은 상자 비율(170/64 = 2.66)이 캔버스와 같아 **그 여백이 곧 가운데 정렬**이다.
    파일을 고치면 PC 가 통째로 왼쪽으로 쏠린다.
