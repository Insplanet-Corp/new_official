...
⚠️ **Careers 입사지원 폼에는 개인정보 동의 절차가 아예 없다** (아직 안 고침).
견적문의 폼에는 `ConsentLinks` + 체크박스가 있고 필수값 게이팅에도 들어 있는데,
`RecruitModal` / `MobileRecruitModal` 에는 동의 문구가 한 줄도 없다 — 이력서는 사진·
생년월일이 담기는 가장 민감한 자료인데 수집 근거가 화면에 없다. 폼을 손볼 때 같이 붙일 것.
