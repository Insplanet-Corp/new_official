...
### 5. 사용자관리 → `admin_users` 프로필 테이블 (`242f774`)
...
`supabase/migrations/001_admin_users.sql` — **아직 실행 안 됨.** 실행 전까지
화면은 "마이그레이션 실행" 안내를 띄운다(`PGRST205` 로 판별, 실제 응답으로 확인함).
RLS 쓰기 정책이 `/admin/users` 권한 보유자만 허용하므로 **최초 1명은 SQL 로 넣어야 한다**
(파일 하단 부트스트랩 insert 주석 참고).

**⚠️ 신규 계정 등록은 브라우저에서 끝낼 수 없다**
- `auth.admin.createUser` → service_role 키 필요, anon 은 403
- `auth.signUp` → 성공하면 **현재 관리자 세션이 새 계정으로 바뀐다**

등록 화면에 대시보드 → SQL → 수정 순서를 안내해 뒀다. 한 번에 처리하려면
`service_role` 키를 쓰는 서버 라우트가 필요하다(미구현).
...
| 메뉴 | 테이블 | 상태 |
|---|---|---|
| 견적문의관리 | `quotes` ✅ | **연동 완료** — 목록·필터·상태변경·상세 |
| 사용자관리 | `admin_users` | 코드 완료, **SQL 실행 대기**. 등록은 서버 라우트 필요 |
| 리크루트관리 | `recruits` | 테이블 없음 |
| 메인관리 | 없음 | 스키마 설계부터 |
| 포트폴리오관리 | `portfolios` (컬럼 `id/created_at/title` 뿐) | 기획서 항목과 안 맞음. 재설계 필요 |

anon 키로 실제 확인한 것: `quotes` ✅ · `portfolios` ✅(스텁) · 나머지는 404(테이블 없음).
