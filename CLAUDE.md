...
**사용자 등록이 조용히 막혀 있던 버그 + 계정 삭제 (2026-09-03)**

- 증상: 사용자관리에서 새 계정을 만들면 **겹치는 ID 가 하나도 없는데 매번
  "이미 사용 중인 ID 입니다"** 로 튕겼다. 목록에도 아무 변화가 없어 원인을 짐작할 수
  없었다(계정이 2개뿐이었다).
- 원인: **`on_auth_user_created` 트리거와 라우트가 같은 행을 두 번 만들려 했다.**
  `POST /api/admin/users` 는 ① Auth 계정 생성 ② `admin_users` 프로필 insert 순인데,
  ①을 하는 순간 002/019 의 트리거가 프로필을 **먼저** 만든다(login_id 는 이메일
  앞부분). 그래서 ②가 **PK 중복**(`23505 · admin_users_pkey`)을 냈고, 라우트가
  23505 를 종류 구분 없이 전부 login_id 중복으로 옮기고 있었다.
  실패하면 롤백이 Auth 계정을 지우는데 FK 가 `on delete cascade` 라 트리거가 만든
  프로필도 같이 사라져 **흔적이 안 남았다.**
  ⚠️ **019 를 실행한 뒤부터 생긴 문제다** — 그 전(새 프로젝트 초기)에는 트리거가
  없어서 등록이 됐다. 계정 2개가 그 시기에 만들어진 것들이다.
- 고침: ②를 `insert` → **`upsert({ onConflict: 'id' })`** 로 바꿔 트리거가 만든
  자리표시 행을 입력값으로 덮는다. 에러 매핑도 제약별로 갈랐다 — 이제
  `admin_users_login_id_key` 일 때만 "이미 사용 중인 ID" 가 뜬다.
  ⚠️ **트리거를 지워서 풀지 말 것** — 대시보드에서 Auth 계정만 만들었을 때 프로필을
  자동 생성하는 장치이고, 없으면 "계정 프로필이 없습니다" 화면이 된다(019 가 되살린 이유).
- ⚠️ **23505 를 뭉뚱그려 "중복" 한 마디로 옮기지 말 것** — 이번 사고가 정확히 그것이었다.
  PK 중복과 unique 컬럼 중복은 원인이 전혀 다르고, 메시지가 엉뚱하면 없는 문제를 찾게 된다.

- **계정 삭제를 붙였다**(사용자 요청 — 기획서 42p 의 미결 항목이 이걸로 해소됐다).
  조회 화면 `/admin/users/<id>` 에 [삭제] 버튼 + `ReasonDialog`(리크루트와 같이
  `withReason={false}` — 계정 삭제를 남길 기록 테이블이 없어 적어도 아무 데도 안 남는다).
  - ⚠️ **지우는 대상은 `auth.users` 다.** 프로필은 FK cascade 로 따라 사라진다.
    프로필만 지우면 로그인은 되는데 프로필이 없는 유령 계정이 남는다.
    그래도 라우트가 프로필 delete 를 **한 번 더** 부른다 — 지역 이전 때처럼 FK 가
    유실된 환경에서는 cascade 가 안 돌아 프로필만 남기 때문이다(보통은 0행).
  - ⚠️ **가드 둘이 잠금을 막는다**: ① 본인 계정 ② `use_yn='Y'` 이면서
    `/admin/users` 권한을 가진 **마지막** 계정. 둘 다 지우면 어드민에서 계정을 만들
    방법이 영영 사라지고, 복구하려면 대시보드에서 계정을 새로 만든 뒤 권한 배열을
    SQL 로 직접 넣어야 한다.
  - **권한 검사는 등록과 공유한다** — `authorizeCaller` 를 `lib/adminUsersApi.ts` 로
    뺐다. ⚠️ route.ts 안에 두고 export 하면 Next 가 라우트 파일의 허용되지 않은
    export 로 보고 빌드에서 걸린다.
- **확인함**(dev 5599, 시험 계정 2개를 만들어 실측하고 전부 지웠다): 토큰 없음 **401** ·
  권한 없는 계정 **403** · 본인 계정 **400** · 정상 삭제 **200** 이고 그 계정이
  `auth.users`·`admin_users` **양쪽에서** 사라짐. upsert 수정은 실제 DB 에서 트리거가
  만든 행이 `name`·`login_id`·`phone`·`permissions` 까지 의도한 값으로 완전히 덮이는
  것을 확인했고, 기존 login_id 와 겹치면 여전히 `admin_users_login_id_key` 로 걸린다.
  사용자가 이 수정으로 **실제 계정 3개(cx · xe · ek.kim) 등록에 성공**했다.
  **확인 못 함**: 마지막-관리자 가드가 실제로 발동하는 장면 — 그걸 보려면 실계정의
  권한을 내려야 해서 시도하지 않았다(가드가 세는 집합이 맞는 것만 확인했다).
  화면에서 사람이 [삭제] 를 눌러 보는 왕복도 세션이 없어 못 봤다.

**Windows 협업 — dev 를 돌릴 때마다 산출물이 수정되던 문제 (2026-09-04)**

- 증상: 저장소를 새로 받은 사람이 `npm run dev` 하면 `src/data/portfolioDetailHtml.json`
  이 수정된 채로 남는다. 원인이 **둘 겹쳐** 있었다.
  1. **커밋된 JSON 이 낡았다** — 커밋 `3eb6061`(kmc 추가)이 `public/portfolio/kmc/` 만
     넣고 재생성된 JSON 을 같이 커밋하지 않았다(37개 vs 실제 38개). **모든 사람에게**
     발생한다. ⚠️ **상세 폴더를 추가하면 이 JSON 을 반드시 함께 커밋할 것.**
  2. **Git for Windows 는 `core.autocrlf` 기본값이 `true`** 라 `index.html` 이 CRLF 로
     체크아웃된다. 생성 스크립트가 파일을 **내용 그대로** 담으므로 JSON 문자열 안에
     `\r\n` 이스케이프가 2,876개 생겨 커밋본과 달라진다. ⚠️ 이건 JSON 안의 이스케이프
     문자라 **git 이 되돌려 주지 못해 영원히 수정 상태로 남는다.**
- 고침: 생성 스크립트가 읽은 내용의 줄바꿈을 **LF 로 정규화**하고(`.replace(/\r\n?/g,"\n")`),
  `.gitattributes` 로 저장소 전체를 `eol=lf` 로 고정했다. 둘 다 둔 이유 — 스크립트 쪽은
  이미 CRLF 로 받아 둔 작업본에서도 즉시 통하고, `.gitattributes` 는 앞으로의 체크아웃을
  막는다.
  ℹ️ 이 JSON 은 `<project-detail>` 속성 파싱에만 쓴다 — 상세 문서 자체는 iframe 이
  `public/` 원본을 직접 읽으므로 줄바꿈을 바꿔도 화면과 무관하다.
- **확인함**: 모든 `index.html` 을 CRLF 로 바꾼 사본(= Windows 체크아웃 재현)에서 스크립트를
  돌려 이 맥의 산출물과 **바이트 단위로 동일**함을 확인. `src/` 에는 파일시스템·경로를
  쓰는 코드가 **한 줄도 없어** 다른 Windows 함정은 없다.

**홈 Our Projects — 창이 낮으면 See More 버튼이 글자를 덮던 문제 (2026-09-04)**

- 증상: 창을 **넓고 낮게** 만들면 `See More Projects` 버튼이 Client/Launch 글자를 덮는다.
- 원인: `.proj-slide`(이름+메타)와 `.proj-more`(버튼)가 **둘 다 `position:absolute`** 였고
  **서로의 존재를 몰랐다.** 슬라이드는 `top`(가로폭 기준 clamp, 최대 458px), 버튼은
  `bottom`(80px)으로 각자 붙어 있어서, 폭이 넓을수록 슬라이드가 아래로 내려가는데
  높이가 낮으면 버튼 자리까지 침범한다. 실측: **1920x820 에서 74.4px 겹침.**
- 고침: `.proj-info` 를 **grid 4행**으로 바꿨다 — ①이름 위 여백(`minmax`) ②슬라이드(3장이
  같은 칸에 겹침) ③`minmax(40px, 1fr)` ④버튼. 공간이 모자라면 **①부터 줄어들고**,
  ③이 최소 40px 간격을 어떤 높이에서도 보장한다.
  ⚠️ 1행 최대값 = 예전 `.proj-slide` 의 `top`, `padding-bottom` = 예전 `.proj-more` 의
  `bottom` 이다. **`>=1900` 블록에도 같은 짝이 있으니 한쪽만 고치지 말 것.**
  ⚠️ 슬라이드를 static 으로 바꿔도 `main.js` 는 안전하다 — opacity/filter/transform 만
  건드리고 위치를 읽지 않는다.
- **확인함**(dev 5599): 1920x1080 · 2560x1440 에서 `nameTop`·바닥여백·`left` 가 **예전과
  동일**(458/80/216, 538/120/218 — 디자인 무변경). 1440x800 · 1024x700 도 무변경.
  높이 1440→620 을 훑어 **겹침 0**(간격이 40px 아래로 안 내려감). 슬라이드 3장이 여전히
  한 칸에 겹치고, `.proj-info` 상자(216~644)가 비주얼 패널(976~)을 침범하지 않아
  "View Project" 클릭 영역도 그대로. 콘솔 에러 0.
...
- ~~사용자 데이터 삭제 기능이 필요한가 (42p)~~ → **2026-09-03 에 붙였다** (조회 화면 [삭제])
