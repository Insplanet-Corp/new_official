...
ℹ️ **첨부파일은 아직 없다.** 한 번 붙였다가 사용자 요청으로 되돌렸다(2026-09-01,
"추후에 다시 넣겠습니다"). 되살릴 때 알아야 할 것 —
  · Contact 문의 폼에는 **파일첨부 칸이 이미 있는데 그 파일이 아무 데도 안 간다**
    (`quotes` 에 컬럼이 없어 두 폼이 조용히 버린다. 에러도 안 난다).
  · 되살리려면 018(recruits)과 같은 구성이면 된다 — 컬럼 3개(`file_path`/`file_name`/
    `file_size`) + 비공개 버킷 + "업로드 → insert" 순서(anon 에게 update 권한이 없다).
  · ⚠️ **비공개 버킷 `quote` 는 이미 만들어져 있다**(Storage API 로 생성, 50MB, 비어 있음).
    정책은 없어서 지금은 anon 업로드가 403 이다. 안 쓸 거면 대시보드에서 지우면 된다.
  · ⚠️ 컬럼을 추가하면 **첨부가 없는 문의에도 `file_*` 를 보내면 안 된다** — 마이그레이션
    전 DB 에서는 PGRST204 로 **문의 접수가 전부 실패한다**(실제로 그래서 방어를 넣었었다).
  · 첨부 다운로드도 열람 기록을 남기려면 `quote_access_logs.action` 에 값을 더해야 한다
    (지금은 `view`/`download` 둘뿐이다).
...
20. **⚠️ `object-position: left` 은 여백이 이미지 **안에** 박혀 있으면 아무 일도 하지 않는다**
    ...
    ⚠️ `crossOrigin="anonymous"` 가 없으면 캔버스가 오염돼 조용히 실패한다(15번과 같은 뿌리).
    ⚠️ **측정 실패를 캐시하지 말 것.** 처음엔 결과를 그대로 WeakMap 에 담았다가, 한 번 오염된
    두 장이 리사이즈·패널 재열기 어디서도 **영영 안 고쳐졌다.** 성공만 캐시하고 실패는 3회까지
    다시 잰다.
    ...
    (적용된 offset -6.47 ~ -25.89px). 1440 에서 PC `.pj-logo img` 는 `50% 50%` 그대로이고
    모바일 트리는 `display:none` 이라 건드리지 않는다. 콘솔 에러 0.
