...
/* ===== custom inverting (difference) cursor =====
   MOVED to src/components/chrome/Cursor.tsx. This runtime is only booted by PageShell, so the
   cursor died on routes that don't use it (/admin/*); it now drives itself from the component. */
...
(function(){
  ...
  if(menuLogo){
    ...
    menuLogo.addEventListener('click',()=>{
      ...
      // "main" = the Next route "/" (was index.html in the static build)
      const onMain=np(location.pathname)==='/';
      if(onMain){
        close();
